...
func TestSanitizeHTML(t *testing.T) {
	tests := []struct {
		...
		{"strips style url()", `<div style="background:url(http://evil/x)">hi</div>`, `<div>hi</div>`},
		{"strips style expression", `<div style="width:expression(alert(1))">hi</div>`, `<div>hi</div>`},
		{"keeps safe style drops unsafe", `<div style="color:blue;background:url(x)">hi</div>`, `<div style="color: blue">hi</div>`},
